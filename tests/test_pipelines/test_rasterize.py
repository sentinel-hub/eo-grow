...
@pytest.mark.chain
@pytest.mark.order(before="test_rasterize_feature_resolution")
@pytest.mark.parametrize("preparation_config,config", [("load_crops_vector_data", "rasterize_feature_shape")])
def test_rasterize_feature_shape(config_and_stats_paths, preparation_config, config):
...
@pytest.mark.chain
@pytest.mark.parametrize("preparation_config,config", [("load_lulc_vector_data", "rasterize_feature_resolution")])
def test_rasterize_feature_resolution(config_and_stats_paths, preparation_config, config):
    preparation_config_path, _ = config_and_stats_paths("rasterize", preparation_config)
    config_path, stats_path = config_and_stats_paths("rasterize", config)
